...
<div style="width:80%; margin:5rem auto; font-family:'Raleway', sans-serif; color:#535353;; font-size:0.9rem">   
    <div style="text-align:center; margin-bottom:1rem">
        ...
        <div><strong>Order Date: {{formatDate($order->created_at)}}</strong></div>
    </div>
        <!--products-->    
        <div style="margin-top:2rem">
                ...
                @foreach($order->products as $product)
                    <div style="display:flex; align-items:center">
                        <div style="width:200px; margin:0">
                            <img src="{{ asset($product->featuredImage())}}" alt="{{$product->name}}" style="width:100%; height:100px; object-fit:contain">
                        </div>
                        <div style="width:calc(100% - 200px); margin:0; display:flex; justify-content:space-between">
                            <div style="text-align:left; margin-left:1rem">
                                <div><a href="{{route('shop.show', $product->slug )}}" style="text-decoration:none; font-weight: 400;font-size: 1rem; color: #989898!important; margin-bottom: 1rem; text-transform:uppercase; text-align:left">{{$product->name}}</a></div>
                                <div style="margin-top: 0.5rem">{{$product->details}}</div>
                            </div>
            ...
            <div style="padding: 1rem;float:right;">
                ...
                <div>{!!displayOrderTotalsForMail($order)!!}</div>
                <div></div>
            </div>
            <!--end Totals-->
        </div>
        <div style="clear:both; display:block; padding-top:3rem; padding-bottom:1rem; text-align:center">
           ...
           <div><strong> You can also check your order status here:</strong></div>
            <a href="{{route('user.showOrder', $order->id)}}" style="display:block;width:10rem; margin:1.5rem auto;text-decoration:none;color:#fff!important;background: #3bb79c;padding: 0.75rem 2.5rem; font-size: 0.95rem;text-transform: uppercase;">My Order</a>
        </div>
        <!--end products-->
</div>
</body>
